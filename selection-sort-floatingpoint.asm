...
for_get:	bge	$s1, $s2, exit_get	# if i>=n go to exit_for_get
		...
		li 	$v0, 6			# Get one element of the array
		syscall				#
		s.s 	$f0, 0($t1)		# The element is stored
						# at the address $t1
		la	$a0, str5
		li	$v0, 4
		syscall
...
exit_get:	move	$a0, $sp		# $a0=base address of the array
...
for_print:	bge	$s1, $s2, exit_print	# if i>=n go to exit_print
		...
		l.s 	$f0, 0($t1)		#
		li	$v0, 2			# print of the element a[i]
		mov.s	$f12,$f0
		syscall				#
...
mini_for:	bgt	$t5, $t2, mini_end	# go to min_end
		...
		c.le.s	$f4,$f7			# if ( min <= v[i] )
		bc1t 	mini_if_exit		# skip the if when v[i] >= min
